...
public class UpanhTratruocFragment extends BaseFragment implements UpanhView {
  @Inject
  UpanhPresenter
  mUpanhPresenter;
  ...
  private void returnUri(int index, Uri image){
    switch (index){
      case BitmapDef.CMND1:
        try {
          loadImageView(mContext,image,imgCmndMattruoc);
          requestBody = UploadUtils.prepareFilePart(mContext,image);
        }catch (Exception e){
          e.printStackTrace();
        }
        break;
      case BitmapDef.CMND2:
        try {
          ...
          requestBody1 = UploadUtils.prepareFilePart(mContext,image);
        }catch (Exception e){
          e.printStackTrace();
        }
        break;
      case BitmapDef.PL4:
        try {
          ...
          requestBody2 = UploadUtils.prepareFilePart(mContext,image);
        }catch (Exception e){
          e.printStackTrace();
        }
        break;
